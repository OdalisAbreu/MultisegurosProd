...
if (isset($action) == "upload") {
	...
	if (file_exists("cop_" . $dates . $archivo)) {
		...
		for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
			...
			if ($i > 1) {
				...
				if ($ValidarAgencia == '1') {
					...
					$insertar2 = "INSERT INTO agencia_via (
		...
		mysql_query($actualizar);*/
					...
					//DESGLOSE DEL REGISTRO
					$actualizar2 = "INSERT INTO auditoria_subida_agencia (
		user_id, archivo, num_agencia , razon_social , estado1 , telefono , ejecutivo , ciudad , municipio, provincia, longitud, latitud, calle,  sector, fecha, tipo 
					...
					if ($num_agencia > 0) {
						$insertar = "INSERT INTO agencia_via (
		user_id, num_agencia , razon_social , estado1 , telefono , ejecutivo , ciudad , municipio, provincia, longitud, latitud, calle,  sector, fecha, activo 
		) VALUES ('$user_id','$num_agencia','$razon_social','$estado1','$telefono','$ejecutivo','$ciudad','$municipio','$provincia','$longitud','$latitud','$calle','$sector','$fecha','$activo')";
